...
export default function Boards() {
  ...
  const getBoardProgress = (boardId) => {
    const boardItems = items.filter(item => item.board_id === boardId);
    if (boardItems.length === 0) return { progress: 0, taskCount: 0 };
    ...
    return {
      progress: Math.round((doneItems / boardItems.length) * 100),
      taskCount: boardItems.length
    };
  };
          ...
          {isLoading ? (
            ...
            <div className={viewMode === "grid" 
              ...
              {filteredBoards.map((board, index) => {
                const { progress, taskCount } = getBoardProgress(board.id);
                return (
                  <BoardCard
                    key={board.id}
                    board={board}
