...
export default function AddResource() {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          toast.success("Location detected! Tap 'Choose on Map' to select a different spot.");
        },
        (error) => {
          console.error("Location error:", error);
          setLocationLoading(false);
          toast.error("Could not detect location. Use 'Choose on Map' to select a location.");
        },
        { enableHighAccuracy: true, timeout: 10000 }
      );
    } else {
      setLocationLoading(false);
      toast.error("Geolocation not supported. Use 'Choose on Map' to select a location.");
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newResource = await base44.entities.ResourceLocation.create({
        ...formData,
      ...
      try {
        await base44.functions.invoke('syncToGoogleSheets', {
          type: 'resource',
          data: { ...newResource, created_by: user.email }
        });
      } catch (sheetError) {
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label className="text-base font-semibold">Resource Location</Label>
              <div className="flex gap-2">
                ...
                <Button 
                  type="button" 
                  variant="outline" 
                  onClick={() => navigate(createPageUrl('ResourceMap'))}
                  className="flex-1"
                >
